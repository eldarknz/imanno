...
function clearCanvas(shape) {
    shape.clearRect(0, 0, shape_canvas.width, shape_canvas.height);
}
...
function inCircle(x, y, r, x1, y1) {
  ...
  if (distanceFromCenter <= r) {
    shape_canvas.style.cursor = 'move';
    is_border = false;
    if (distanceFromCenter >= r - 10 && distanceFromCenter <= r) {
  ...
  if (is_resize_shape == true) {
    switch (selected_shape.type) {
      ...
      case 'circle':
        ...
        break;
      case 'ellipse':
        var inX0 = Math.sqrt(Math.pow((selected_shape.x + selected_shape.x1) - currentX, 2) + Math.pow(selected_shape.y - currentY, 2))
        var inY0 = Math.sqrt(Math.pow((selected_shape.y + selected_shape.y1) - currentY, 2) + Math.pow(selected_shape.x - currentX, 2))
        var inX1 = Math.sqrt(Math.pow((selected_shape.x - selected_shape.x1) - currentX, 2) + Math.pow(selected_shape.y - currentY, 2))
        var inY1 = Math.sqrt(Math.pow((selected_shape.y - selected_shape.y1) - currentY, 2) + Math.pow(selected_shape.x - currentX, 2))
        if ((inX0 <= 10) || (inX1 <= 10) && (currentXY == null)) { currentXY = 'x'; }
        if ((inY0 <= 10) || (inY1 <= 10) && (currentXY == null)) { currentXY = 'y'; }
        if (currentXY == 'x') {
            selected_shape.x1 = Math.abs(currentX - selected_shape.x + 2);
        }
        if (currentXY == 'y') {
            selected_shape.y1 = Math.abs(currentY - selected_shape.y + 2);
        }
        break;
      /*case 'polygon':
        console.log('polygon');
