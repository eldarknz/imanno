...
// admission to drawing shape
var start_drawing_shape    = false;
// admission to drawing brushstroke
var start_drawing_brush    = false;
...
// polygon perimeter
var perimeter              = [];
// previous selected shape
var prev_selected_shape    = null;
// previous focused shape
var prev_focused_shape     = null;
//
...
function transformShape() {
  ...
  // check dragging
  if (is_resizing == true) {
    switch (prev_selected_shape.type) {
      case 'rect':
        prev_selected_shape.h = currentY - prev_selected_shape.y;
        prev_selected_shape.w = currentX - prev_selected_shape.x;
        break;
      case 'circle':
        prev_selected_shape.r = Math.sqrt(Math.pow(prev_selected_shape.x - currentX, 2) +
                                          Math.pow(prev_selected_shape.y - currentY, 2))
        break;
      case 'ellipse':
        console.log('ellipse');
        break;
  ...
  if (is_dragging == true) {
    // check in shape
    if (prev_selected_shape != null) {
      // move the shape to a new position
      if (prev_selected_shape.type == 'polygon') {
        for (var i = 0; i < prev_selected_shape.coordinates.length; i++) {
          prev_selected_shape.coordinates[i].x = currentX - deltaXYPoly[i].dx;
          prev_selected_shape.coordinates[i].y = currentY - deltaXYPoly[i].dy;
        }
      } else {
          prev_selected_shape.x = currentX - deltaX1;
          prev_selected_shape.y = currentY - deltaY1;
      }
    }
  }
  clearCanvas();
...
/*function transformShape() {
  ...
  if (is_dragging == true) {
    ...
    if (prev_selected_shape != null) {
      ...
      } else if (prev_selected_shape.type == 'circle') {
        if (!is_resizing) {
          prev_selected_shape.x = currentX - deltaX1;
          prev_selected_shape.y = currentY - deltaY1;
        } else {
          var x = prev_selected_shape.x;
          var y = prev_selected_shape.y;
          prev_selected_shape.r = Math.sqrt(Math.pow(x - currentX, 2) + Math.pow(y - currentY, 2))
        }
      } else {
        if (!is_resizing) {
          prev_selected_shape.x = currentX - deltaX1;
          ...
          console.log(prev_selected_shape.h);
          console.log(prev_selected_shape.w);
          prev_selected_shape.h = currentY - prev_selected_shape.y;
          prev_selected_shape.w = currentX - prev_selected_shape.x;
        }
      }
  ...
  if(list_shapes.length > 0) {
    for (var i = 0; i < list_shapes.length; i++) {
      ...
      if (inShape(shape, x, y)) {
        ...
        if (prev_selected_shape != null) {
          prev_selected_shape.isSelected = false;
        }
        selected_shape                   = shape;
        prev_selected_shape              = shape;
  ...
  if(list_shapes.length > 0) {
    for (var i = 0; i < list_shapes.length; i++) {
      ...
      if (inShape(shape, x, y)) {
        ...
        prev_focused_shape = shape;
        return;
      } else {
        shapes_canvas.style.cursor = 'crosshair'
        prev_focused_shape = null;
      }
    }
  }
}
...
shapes_canvas.addEventListener('mousedown', function(event) {
  ...
  /*console.log('start_drawing_shape => ' + start_drawing_shape);
  console.log('start_drawing_brush => ' + start_drawing_brush);
  ...
  console.log('is_resizing => ' + is_resizing);*/
}, false);
...
function startDrawing(type) {
  ...
  if (type == 'brushstroke') {
    ...
    if (btn_brushstroke.classList.contains('active')) {
      ...
      start_drawing_brush = true;
      start_drawing_shape = false;
      if (is_selected) {
        prev_selected_shape = null;
        is_dragging         = false;
        is_selected         = false;
